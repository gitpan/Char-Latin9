...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin9::uc($char[$i]) ne Char::Elatin9::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin9::uc($char[$i]) . Char::Elatin9::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin9/Char::Latin9/JA.pm
...
Definitions in Char/Elatin9.pm.
  ...
  @{Char::Elatin9::eV}             qr{(?:[^\x0C\x0A\x0D])}
