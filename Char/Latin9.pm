...
######################################################################
#
# Char::Latin9 - Source code filter to escape Latin-9 script
#
# http://search.cpan.org/dist/Char-Char::Latin9/
#
...
BEGIN { CORE::require Char::Elatin9; }
...
my $function_ord;         # ord()   to ord()   or Char::Latin9::ord()
my $function_ord_;        # ord     to ord     or Char::Latin9::ord_
my $function_reverse;     # reverse to reverse or Char::Latin9::reverse
my $function_getc;        # getc    to getc    or Char::Latin9::getc
...
END
    ...
    print Char::Latin9::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Elatin9::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Latin9::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Elatin9::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Latin9::escape_script {
    ...
    # read Latin-9 script
    my $fh = gensym();
    Char::Elatin9::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Elatin9(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Elatin9 %s;\n", $Char::Latin9::VERSION); # require run-time routines version

        # use Char::Latin9 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Latin9 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Latin9::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Latin9.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/Latin9.pm $version, but @{[__FILE__]} is version $Char::Latin9::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Elatin9.pm $version, but \\\$Char::Elatin9::VERSION is \$Char::Elatin9::VERSION" if \$Char::Elatin9::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Latin9::VERSION) {
                    die "Script $0 required Char/Latin9.pm $version, but @{[__FILE__]} is only version $Char::Latin9::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Latin9::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Latin9::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Latin9::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Latin9::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Char::Elatin9
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Latin9::re_t=%s; while($Char::Latin9::re_t =~ %s){%s local $^W=0; local $Char::Latin9::re_r=%s; %s$Char::Latin9::re_t="$`$Char::Latin9::re_r$'"; pos($Char::Latin9::re_t)=length "$`$Char::Latin9::re_r"; } return $Char::Latin9::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Latin9::re_r=eval $Char::Latin9::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Latin9::re_r=%s; %s"%s$Char::Latin9::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Latin9::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Latin9::re_r=%s; %s%s="$`$Char::Latin9::re_r$'"; pos(%s)=length "$`$Char::Latin9::re_r"; $Char::Latin9::re_n++} return %s$Char::Latin9::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Latin9::re_r=%s; %s%s="%s$Char::Latin9::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Latin9::re_r=%s; %s%s="%s$Char::Latin9::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Latin9::re_r=eval $Char::Latin9::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Latin9.pm and Char/Elatin9.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Latin9;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Latin9;
  use Char::Latin9 ver.sion;             --- require minimum version
  use Char::Latin9 ver.sion.0;           --- expects version (match or die)

  # "no Char::Latin9;" not supported

  or

  $ perl Char/Latin9.pm Latin-9_script.pl > Escaped_script.pl.e
...
Char::Latin9 software is "middleware" between perl interpreter and your Perl script
written in Latin-9.
...
By "use Char::Latin9;", it automatically interpret your script as Latin-9. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Latin9.pm, Char/Elatin9.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Elatin9.pm
works well.
...
=head1 Software Composition
   ...
   Char/Latin9.pm               --- source code filter to escape Latin-9
   Char/Elatin9.pm              --- run-time routines for Char/Latin9.pm
...
You need write 'use Char::Latin9;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Latin9;
  ---------------------------------

=head1 Calling 'Char::Elatin9::ignorecase()' (Char/Latin9.pm provides)

Char/Latin9.pm applies calling 'Char::Elatin9::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Elatin9::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Elatin9.pm provides)
...
Definitions in Char/Elatin9.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Latin9.pm provides)

Char/Latin9.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Latin9.pm and Char/Elatin9.pm provide)

Insert 'Char::Elatin9::' at head of function name. Char/Elatin9.pm provides your script Char::Elatin9::*
functions.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Elatin9::lc        Character
  lcfirst     Char::Elatin9::lcfirst   Character
  uc          Char::Elatin9::uc        Character
  ucfirst     Char::Elatin9::ucfirst   Character
  fc          Char::Elatin9::fc        Character
  chr         Char::Elatin9::chr       Character
  glob        Char::Elatin9::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Functions (Char/Latin9.pm provides)

Char/Latin9.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Elatin9.pm provides)

Char/Elatin9.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Latin9' }" at head.
Store the standard module modified for Char::Latin9 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Latin9 and change
'use utf8;' to 'use Char::Latin9;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Elatin9.pm provides these
  functions.
...
Char::Latin9 software remains one language and one interpreter.
