...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Elatin9::uc($char[$i]) ne Elatin9::lc($char[$i]))) {
            $char[$i] = '[' . Elatin9::uc($char[$i]) . Elatin9::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Latin9/Latin9/JA.pm
