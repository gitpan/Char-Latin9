...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and (Elatin9::uc($char[$i]) ne Elatin9::lc($char[$i]))) {
...
=item * Only Latin9.pm and Elatin9.pm, other modules are unnecessary
...
functions.
  ...
  ---------------------------------
  lc          Elatin9::lc
  lcfirst     Elatin9::lcfirst
  uc          Elatin9::uc
  ucfirst     Elatin9::ucfirst
  chr         Elatin9::chr
  glob        Elatin9::glob
  ---------------------------------
...
Latin9.pm remove 'bytes::' at head of function name.
...
=head1 Perl5.6 Emulation on perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Elatin9.pm
  --------------------------------------------------------------------
  binmode(...);   Elatin9::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Elatin9::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005, Latin9 software emulate perl5.6's binmode function.
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  open(FH, "<:crlf:encoding(Latin9)", $path) or die "can't open $path: $!";
...
=head1 Ignore utf8 pragma
  ...
  ---------------------------------------------------------------------
  use utf8;       # use utf8;            Elatin9.pm provides utf8::*
  no utf8;        # no utf8;             functions even if 'no utf8;'
  use bytes;      # use bytes;           Elatin9.pm provides bytes::*
