...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Elatin9::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Elatin9::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Elatin9::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Elatin9::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Elatin9::split returns
...
=item Chop string
  ...
  The Elatin9::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Elatin9::index($string,$substr,$position);
  $pos = Elatin9::index($string,$substr);
  ...
  the Elatin9::index function returns -1.

=item Reverse index string

  $pos = Elatin9::rindex($string,$substr,$position);
  $pos = Elatin9::rindex($string,$substr);

  Works just like Elatin9::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Elatin9::chr(65) is "A" in either ASCII or Latin-9, not Unicode,
  and Elatin9::chr(0x82a0) is a Latin-9 HIRAGANA LETTER A. For the reverse of
  Elatin9::chr, use Latin9::ord.
